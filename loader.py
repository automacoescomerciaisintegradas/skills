...
class SkillLoader:
    ...
    def load_skill(self, skill_dir: Path) -> Optional[BaseSkill]:
        ...
        try:
            ...
            module_name = f"skills.{skill_dir.parent.name}.{skill_dir.name}.{skill_file.stem}"
    ...
    def get_all_definitions(self) -> str:
        """
        Gera o texto de ajuda para o System Prompt do LLM.
        
        Returns:
            String com todas as definições de skills
        """
        definitions = []
        for skill in self.skills.values():
            if hasattr(skill, 'get_definition'):
                definitions.append(skill.get_definition())
        return "\n\n".join(definitions)
...
if __name__ == "__main__":
    print("=== Testando SkillLoader Universal ===\n")
    ...
    print(f"   Encontradas: {len(skill_dirs)} skills\n")
    ...
    for info in loader.list_skills():
        status = "✅" if info["has_implementation"] else "📄"
