...
- **Manifest leggero**: usare `data/skills_index.json` para saber *quais* skill existem, sin caricar i testi completi.
...
- **`id`**: identificatore usado nelle menzioni `@id` (es. `@brainstorming`).
...
- spostate le skill non necessarie in `skills/.disabled/` per escluderle in certi ambientes;
